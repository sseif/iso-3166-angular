...
angular.module('iso-3166', [])
    .factory('ISO3166', function() {
        ...
        for (var key in holder.codeToState) {
            holder.stateToCode[holder.codeToState[key]] = key;
            holder.stateCodes.push(key);
        }

        holder.isStateCode = function(input) {
            if (angular.isString(input)) {
                input = input.toUpperCase();
            }
            return angular.isDefined(this.codeToState[input]);
        };

        holder.getStateCode = function(stateName, manipulator) {
            var stateCode = this.stateToCode[StateName.toUpperCase()];
            manipulator = manipulator ? manipulator : 'toUpperCase';

            return stateCode && stateCode[manipulator]();
        };

        holder.getStateName = function(stateCode, manipulator) {
            manipulator = manipulator ? manipulator : 'toString';

            return this.codeToState[stateCode] && this.codeToState[stateCode][manipulator]();
        };

        holder.getStateNames = function(stateCodes, manipulator) {
            manipulator = manipulator ? manipulator : 'toString';

            var states = {};
            angular.forEach(stateCodes, function(key) {
                if (holder.isStateCode(key)) {
                    states[key] = holder.getStateName(key, manipulator);
                }
            });
            return states;
        };
    ...
    .filter('isoState', ['ISO3166', function(ISO3166) {
        return function(input) {
            var result = ISO3166.getStateName(input);
            return angular.isUndefined(result) ? input : result;
        };
    }])
    .filter('isoStateCode', ['ISO3166', function(ISO3166) {
        return function(input) {
            var result = ISO3166.getStateCode(input);
            return angular.isUndefined(result) ? input : result;
        };
    }])
    .directive('stateCode', ['ISO3166', function(ISO3166) {
        return {
            require: 'ngModel',
            link: function(scope, elm, attrs, ctrl) {
                ctrl.$parsers.unshift(function(viewValue) {
                    if (ISO3166.isStateCode(viewValue)) {
                        ctrl.$setValidity('statecode', true);
                        return viewValue;
                    } else {
                        ctrl.$setValidity('statecode', false);
                        return undefined;
                    }
                });
            }
